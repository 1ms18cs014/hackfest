...
MYSQL_DB=os.environ.get('MYSQL_DB', 'rsvp')
MYSQL_USER=os.environ.get('MYSQL_USER', 'root')
MYSQL_PASS=os.environ.get('MYSQL_PASS', 'root')

time.sleep(2)
db = mysql.connector.connect(user = MYSQL_USER, password = MYSQL_PASS,
                              host = MYSQL_HOST, database = MYSQL_DB,
                              auth_plugin='mysql_native_password')

# Create table if it doesn't exist
db_cursor = db.cursor()
tbl = """CREATE TABLE IF NOT EXISTS entries (
         id INT AUTO_INCREMENT PRIMARY KEY,
         name VARCHAR(30) NOT NULL,
         email VARCHAR(50) NOT NULL)"""
db_cursor.execute(tbl)
db_cursor.close()
...
def rsvp():
    ...
    _items = db_cursor.fetchall()
    items = []
    # tuples are in the format (id, name, email)
    for item in _items:
